...
// scrapeFamilyCard.js
// 
//     Scrape a Family Search person's detail page. This returns a json
// object of the form:
// 
// {
//   source:{
...
//   },
//   people: [ /* list of pIDs */],
//   peopleMap: {'pID1': {/* attributes */}, ...},
//   families: [ /* list of family IDs */],
//   familyMap: {'pID1_pID2': { husband: 'pID1, wife, marriage, children:[]}}
// }
//
// 
...
export class FamilyCard {
  ...
  static jsonForField(fld, src) {
    ...
    if (!el) return "";
    if (fld.type === 'event')
      return FamilyCard.scrapeFields('event', el);
  ...
  static scrapeFields(fieldType, src) {
    ...
    const fieldTypes = {
      ...
      'vitals': [
        {tagName: "fullname", type: 'text', selectorTags: ["conclusionDisplay:NAME", "conclusion-body"]},
        {tagName: "gender", type: 'text', selectorTags: ["conclusionDisplay:GENDER", "conclusion-gender"]},
        {tagName: "birth", type: 'event', selectorTags: ["conclusionDisplay:BIRTH"]},
        {tagName: "christening", type: 'event', selectorTags: ["conclusionDisplay:CHRISTENING"]},
        {tagName: "death", type: 'event', selectorTags: ["conclusionDisplay:DEATH"]},
        {tagName: "burial", type: 'event', selectorTags: ["conclusionDisplay:BURIAL"]},
      ]
    }
  ...
  #addFamily({husband, wife, marriage, children}) {
    ...
    children.forEach(
      (child) => this.#addPerson({...child, childOf: [familyID] })
    )
  }
  ...
  #scrapeFamily(familyNode) {
    ...
    if (childrenNode) {
      ...
      familiesChildren.forEach(
         (child) => children.push(FamilyCard.scrapeFields('person', child))
       );
     }
  ...
  #scrapeFamilyMembers() { // also scrapes individuals
  ...
  constructor () {
    if (!document.URL.includes('details/')) {
      alert("Downloading GEDCOM data can only be done on the Details page of a FamilySearch person's page.");
    }
...
export class FamilyToString {
  // for converting a family card object to plain text
  constructor (family) {
    this.family = family
  }
  static shortToday(date) { // e.g. "Jun 27, 2024"
    const options = {
      day: 'numeric',
      month: 'short',
      year: 'numeric'
  ...
  static personToString(person) {
    ...
    if (person.lifespan && person.lifespan.length > 0) str += ` (${person.lifespan},`;
    str += ` ${person.pID})`;
    return str;
  }
  ...
  static pushFamily(strs, family, peopleMap) {
    ...
    if (children.length > 0) {
      strs.push("Children:");
      children.forEach(
        (childID) => strs.push('  ' + FamilyToString.personToString(peopleMap[childID]))
      );
    }
    strs.push("");
  }
  toString() {
    ...
    strs.push(FamilyToString.eventToString("Christening", sourcePerson.christening));
    ...
    if (sourceFamilies.length > 0) {
      strs.push("Families:");
      sourceFamilies.forEach(
        (famID) => FamilyToString.pushFamily(strs, familyMap[famID], peopleMap)
      );
    }
    if (parents.length > 0) {
...
export class Gedcom {
  ...
  processPeople(people, peopleMap) {
    people.forEach(
      (pID) => {
        ...
        this.individuals.dict[indvId] = this.recsForPerson(indvId, peopleMap[pID]);
      }
    )
  }
  processMarriages(families, familyMap) {
    families.forEach(
      (pID) => {
        ...
        this.couples.dict[coupleId] = this.recsForCouple(coupleId, familyMap[pID]);
      }
    )
  ...
  eventToGed(tagName, evt) {
    ...
    const sanitizeDate = (dateString) => {
      ...
      return dateString.replace(/about/gi, "ABT")
  ...
  addCoupleToGed(marriage, vitals=null) {
    ...
    // add family
    const familyId = `${husband.pID}_${wife.pID}`;
    if (!this.gedMap.families[familyId])
...
// downloadFile(`${family.source.title}.txt`, family.toString());
